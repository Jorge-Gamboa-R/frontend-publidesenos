import type { ThemeAccent } from '../types';

type Palette = Record<'50' | '100' | '200' | '300' | '400' | '500' | '600' | '700' | '800' | '900', string>;

export const ACCENT_PALETTES: Record<ThemeAccent, Palette> = {
  purple: {
    50: '#faf5ff', 100: '#f3e8ff', 200: '#e9d5ff', 300: '#d8b4fe', 400: '#c084fc',
    500: '#a855f7', 600: '#9333ea', 700: '#7e22ce', 800: '#6b21a8', 900: '#581c87',
  },
  blue: {
    50: '#eff6ff', 100: '#dbeafe', 200: '#bfdbfe', 300: '#93c5fd', 400: '#60a5fa',
    500: '#3b82f6', 600: '#2563eb', 700: '#1d4ed8', 800: '#1e40af', 900: '#1e3a8a',
  },
  green: {
    50: '#f0fdf4', 100: '#dcfce7', 200: '#bbf7d0', 300: '#86efac', 400: '#4ade80',
    500: '#22c55e', 600: '#16a34a', 700: '#15803d', 800: '#166534', 900: '#14532d',
  },
  pink: {
    50: '#fdf2f8', 100: '#fce7f3', 200: '#fbcfe8', 300: '#f9a8d4', 400: '#f472b6',
    500: '#ec4899', 600: '#db2777', 700: '#be185d', 800: '#9d174d', 900: '#831843',
  },
};

export const ACCENT_LABELS: Record<ThemeAccent, string> = {
  purple: 'Morado',
  blue: 'Azul',
  green: 'Verde',
  pink: 'Rosa',
};

export const STORAGE_KEY = 'themeAccent';

export const applyAccent = (accent: ThemeAccent) => {
  const palette = ACCENT_PALETTES[accent];
  const root = document.documentElement;
  for (const [shade, hex] of Object.entries(palette)) {
    root.style.setProperty(`--color-primary-${shade}`, hex);
  }
};

export const readStoredAccent = (): ThemeAccent => {
  const stored = localStorage.getItem(STORAGE_KEY);
  if (stored === 'purple' || stored === 'blue' || stored === 'green' || stored === 'pink') {
    return stored;
  }
  return 'purple';
};
